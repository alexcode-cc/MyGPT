...
|------|------|------|
| `text` | string | 完整轉錄文字 |
| `language` | string | Whisper 偵測的主要語言 |
| `languages` | string[] | 文字中實際包含的所有語言（多語言分析） |
| `language_probability` | number | 語言偵測的信心分數 |
| `duration` | number | 音檔總時長（秒） |
| `duration_after_vad` | number | VAD 處理後的時長（秒） |
| `segments` | array | 轉錄片段（含時間戳） |
...
| 音檔轉錄 | 📁 | faster-whisper | 高品質，支援多種格式，自動偵測多語言 |

### 多語言偵測

faster-whisper 服務會分析轉錄文字中的 Unicode 字符，自動偵測包含的語言：
...
例如，包含英語和日語的歌曲會返回 `languages: ["en", "ja"]`。
